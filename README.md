...
4. 可以支持SQL中包含 between 和 not between 操作符，并且避免SQL路由到所有节点
5. 改进auto-sharding-long分片算法，不会路由到错误节点查不出数据，也不会不必要的路由到所有节点影响性能
6. 解析并识别出范围查询（x > 0 and x < 100），精确实现最优化的SQL路由 
7. 简单重构优化代码减少对象创建
